  ...
  static Future<HttpClient> customHttpClient({
    ...
    try {
      ...
      if (isTestMode) {
        ...
        bytes = (await rootBundle.load('cert.pem')).buffer.asUint8List();
      }
...
class SSLPinningClient extends IOClient {
  ...
  Future<http.Response> get(Uri url, {Map<String, String>? headers}) async {
    return await Sharing.initializeIOClient().then((value) => value.get(url));
  }
}

class Sharing {
  static Future<IOClient> initializeIOClient() async {
    final ByteData sslCert = await rootBundle.load('certificate.cer');
